  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#150A5E] text-[#FAF9F5] border-r border-[#FAF9F5]/10">
      ...
      <nav 
        ...
        className="flex-grow px-3 py-4 flex flex-col gap-5 overflow-y-auto custom-scrollbar"
      >
        {groupedNavItems.length === 0 ? (
          ...
          groupedNavItems.map((group) => (
            <div key={group.category} className="flex flex-col gap-1">
              <div className="px-3 pb-1 text-[10px] font-semibold text-[#FAF9F5]/50 uppercase tracking-widest">
                {group.category}
              </div>
              {group.items.map((item) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      {isActive && (
                        <span className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center text-[#C99A3B]">
                          <svg width="4" height="14" viewBox="0 0 4 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect width="4" height="14" rx="2" fill="currentColor" />
                          </svg>
                        </span>
                      )}
                      <span className={isActive ? "text-[#C99A3B] pl-1" : "pl-1"}>{ICON_MAP[item.id]}</span>
